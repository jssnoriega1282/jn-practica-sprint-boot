package com.pruebapractica.apitestdocker.comunes;
...
public abstract class GenericServiceImpl<I, O> implements GenericServiceAPI<I, O> {
	...
	public Class<I> clazi;

	public GenericServiceImpl() {
		this.clazz = ((Class<O>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[1]);
		this.clazi = ((Class<I>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0]);
	}
	...
	@Override
	public List<O> findByEdadAndEstado(Integer edad, String estado) throws Exception {
	...
	@Override
	public I getEntity(String id) throws Exception {
		DocumentReference ref = getCollection().document(id);
		ApiFuture<DocumentSnapshot> futureDoc = ref.get();
		DocumentSnapshot document = futureDoc.get();
		if (document.exists()) {
			I object = document.toObject(clazi);
			return object;
		}
		return null;
	}
